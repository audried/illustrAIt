...
export const generatePrompt = (tracks, artists) => {
    ...
    const toptracks = tracks.map(item =>{return item.name})
    const topalbums = tracks.map(item =>{return item.album.name})
    ...
    const formats = [", digital art", ", 3d render", ", 5mm film photograph", ", acrylic painting",  ", oil painting", ", hyperrealistic", ", ukioh-e", ", watercolor"]
    ...
    var chosen_track = toptracks[randomtrack]
    toptracks.splice(randomtrack,1)
    var second_track = toptracks[Math.floor((Math.random()*tracks.length-1))]
    //var chosen_album = fixTitle(topalbums[Math.floor((Math.random()*5))])
    
    
    console.log(chosen_track, second_track, toptracks.length)

    //builds dalle prompt
    //replace second_track with chosen_album for song and album instead of 2 songs
    var query = fixTitle(chosen_track) + ' ' + fixTitle(second_track) + subject
    ...
    //can replace second_track with chosen_album
    var caption = " AI generated art inspired by " + chosen_track + ",  " + second_track + ", and " + top_genre + " music"
    if (chosen_track.length > 29){ chosen_track = fixTitle(chosen_track)}
    if (second_track.length > 29){ second_track = fixTitle(second_track)}

    return ([query, caption, cutOff(chosen_track, 29), cutOff(second_track, 29), top_genre])
}
