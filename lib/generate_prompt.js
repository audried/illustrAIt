...
export const generatePrompt = (tracks, artists) => {
    ...
    let randomtrack = Math.floor((Math.random()*5))
    var chosen_track = top5tracks[randomtrack]
    top5tracks.splice(randomtrack,1)
    var second_track = top5tracks[Math.floor((Math.random()*4))]
    var chosen_album = fixTitle(top5albums[Math.floor((Math.random()*5))])
    
    

    console.log(chosen_track, second_track, top5tracks.length)
