...
const getAccessToken = async (refresh_token) => {
  ...
  export const getUsersTopArtists = async (refresh_token, tr) => {
    const {access_token} = await getAccessToken(refresh_token);
    return fetch(`${ARTISTS_ENDPOINT}?time_range=${tr}`, {
      headers: {
  ...
  export const getUsersTopTracks = async (refresh_token, tr) => {
    const {access_token} = await getAccessToken(refresh_token);
    return fetch(`${TRACKS_ENDPOINT}?time_range=${tr}`, {
      headers: {
        Authorization: `Bearer ${access_token}`,
      },
    });
