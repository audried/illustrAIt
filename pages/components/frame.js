...
export function Frame(props){
    ...
    useEffect(() => {
        ...
        image.onload = () => {
            ...
            context.font = width==500 ? '12px Special Elite' : '9px Special Elite'
...
// //canvas responsiveness: https://stackoverflow.com/questions/8625208/working-with-canvas-in-different-screen-sizes 
// //https://developer.mozilla.org/en-US/docs/Web/HTML/CORS_enabled_image
// //font fix: https://stackoverflow.com/questions/2756575/drawing-text-to-canvas-with-font-face-does-not-work-at-the-first-time
...
//     if (!big && window.innerWidth >= 550){
//         setBig(true)
//     }else if (big && window.innerWidth <550){
//         setBig(false)
//     }
//     console.log(big)
...
//                 if (!isVisible){
//                     context.drawImage(image, 0, 0, width, width);
//                     context.font = big ? '50px Special Elite' : '40px Special Elite'
//                     context.fillStyle ='#000000'
//                     context.fillText(`IMAGE #${img_id}`, 10, .78*height);
//                     context.font = big ? '24px Special Elite' : '16px Special Elite'
//                     context.fillText(props.chosen[0]+",", .08*width, .84*height);
//                     context.fillText(props.chosen[1]+",", .08*width, .88*height);
//                     context.fillText("and "+props.chosen[2] +" music", .08*width, .924*height);
//                     context.font = big ? '12px Special Elite' : '8px Special Elite'
//                     context.textAlign = 'center'
//                     context.fillText("create AI generated art based on your music taste at illustrait.us", center, .97*height);
//                     setIsVisible(true)}
//             };
...
//             {/* purpose of Text is to load font */}
//             <Text className={styles.loadFont}>.</Text> 
//              <canvas id="canvas" ref={canvasRef} width={window.innerWidth>=550 ? 500 : .9*window.innerWidth} height={window.innerWidth>=550 ? 720 : 1.44*.9*window.innerWidth} style={ isVisible ? {} : { visibility: "hidden" } }/>
//             <Button onClick={download} rounded={'full'} px={6} size='lg' m={'5'} className={styles.db}>Download image</Button>
//             <image id="theimage"></image>
//         </div>
